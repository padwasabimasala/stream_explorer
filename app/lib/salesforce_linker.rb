class SalesforceLinker
  ...
  def links 
    ...
    if @message.data && @message.data.dig('orderItems', 0, 'orderId')
      return {'Order' => url_base + 'Order/' + @message.data.dig('orderItems', 0, 'orderId') + '/view'}
    end
    if @message.data && @message.data.dig('accountId') && @message.data.dig('accountId').length == 18
      return {'Account' => url_base + 'Account/' + @message.data.dig('accountId') + '/view'}
    end
    {}
